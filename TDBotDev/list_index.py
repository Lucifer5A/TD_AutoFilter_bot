...
async def start_nav_flow(client, message, series, files, is_cb=False):
    ...
    # 2) Language Detection & Auto-skip
...
async def season_flow(client, message, series, lang, qual, season, files, is_cb):
    ...
    # 5) EPISODE DISPLAY: Clean file name instead of E01/E02
    episodes = []
    for f in final_files:
        # Extract episode number for sorting only
        match = EPISODE_REGEX.search(f['file_name'])
        e_num = int(match.group(2)) if match else 1
        episodes.append({
            "id": str(f["_id"]),
            "ui_name": clean_ui_name(f['file_name']), # Rule 5
            "e_num": e_num
        })
    ...
    buttons = []
    for ep in episodes:
        buttons.append([InlineKeyboardButton(ep["ui_name"], callback_data=f"f#{ep['id']}")])
