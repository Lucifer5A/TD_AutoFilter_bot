...
def pack_cb(q, qu, l, pg):
    data = f"page#{q}#{qu}#{l}#{pg}"
    if len(data) > 64:
        limit = 64 - (5 + 1 + len(str(qu)) + 1 + len(str(l)) + 1 + len(str(pg)))
        data = f"page#{q[:limit]}#{qu}#{l}#{pg}"
    return data
...
def get_ui(q, qu, l, pg, total, results):
    ...
    nav = []
    if pg > 0:
        nav.append(InlineKeyboardButton("⬅️ Prev", callback_data=pack_cb(q, qu, l, pg - 1)))
    if (pg + 1) * MAX_RESULTS < total:
        nav.append(InlineKeyboardButton("Next ➡️", callback_data=pack_cb(q, qu, l, pg + 1)))
    if nav:
        buttons.append(nav)
...
async def initial_search_handler(client, message: Message):
    # Check indexing status from app state
...
async def filter_menu_handler(client, cb: CallbackQuery):
    ...
    if m_type == "q":
        for opt in ["480p", "720p", "1080p"]:
            buttons.append([InlineKeyboardButton(opt, callback_data=pack_cb(q, opt, l, 0))])
    else:
    ...
    buttons.append([InlineKeyboardButton("🔙 Back", callback_data=pack_cb(q, qu, l, pg))])
